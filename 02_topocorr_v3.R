topocorr_v3 <-function(x, slope, aspect, sunelev, sunazimuth, method="cosine", na.value=NA, GRASS.aspect=FALSE, IL.epsilon=0.000001) {
    ...
    if(method == 1){
      ...
      ## C correction with smoothing factor (use factor of 5; Raino et al. 2003)
      slope_smooth <- atan(0.2*tan(slope))
      IL_smooth <- cos(slope_smooth) * cos(sunzenith) + sin(slope_smooth) * sin(sunzenith) * cos(sunazimuth - aspect)
      IL_smooth[IL_smooth == 0] <- IL.epsilon
      
      band.lm <- lm(as.vector(x) ~ as.vector(IL_smooth))
      C <- coefficients(band.lm)[[1]]/coefficients(band.lm)[[2]]
      
      xout <- x * (cos(sunzenith) + C) / (IL_smooth + C)
    }
    else if(method == 8) {
      ## Empirical Statistical (see Hantson & Chuvieco, 2011 IJAEOG)
      band.lm <- lm(as.vector(x) ~ as.vector(IL))
      b <- coefficients(band.lm)[[1]]
      m <- coefficients(band.lm)[[2]]
